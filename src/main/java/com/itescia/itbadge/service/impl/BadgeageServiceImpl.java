...
public class BadgeageServiceImpl implements BadgeageService {
    ...
    public Badgeage save(Badgeage badgeage) {
        log.debug("Request to save Badgeage : {}", badgeage);
        return badgeageRepository.save(badgeage);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public Badgeage findOne(Long id) {
        log.debug("Request to get Badgeage : {}", id);
        return badgeageRepository.findOne(id);
    }
